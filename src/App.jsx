...
import { 
  ...
  Crosshair, Zap, Menu, Building, Lock, KeyRound, Plus, Trash2, Cpu, Link as LinkIcon, Crosshair as GpsIcon
} from 'lucide-react';
...
// --- Gemini API Setup ---
const geminiApiKey = ""; // Environment provided
...
// --- Notification & Hardware Alarm Helper ---
const triggerHardwareAlarm = () => {
  // 1. Vibrate device (if supported, mostly Android)
  if ("vibrate" in navigator) {
    navigator.vibrate([500, 200, 500, 200, 500]);
  }
  // 2. Play synthetic siren using Web Audio API (Bypasses some media restrictions)
  try {
  ...
  } catch (e) { console.warn("Audio alarm blocked by browser policy"); }
};
...
export default function App() {
  ...
  // App State & Routing
  const [currentRole, setCurrentRole] = useState('portal'); // 'portal' | 'guest' | 'staff' | 'responder' | 'admin'
  ...
  useEffect(() => {
    ...
    const unsubVenues = onSnapshot(venuesRef, (snapshot) => {
      ...
      // Check URL for Deep Linking (QR Code Simulation)
  ...
  // Periodic alarm if alerts are active
  useEffect(() => {
    let alarmInterval;
    ...
    if ((currentRole === 'staff' || currentRole === 'responder') && activeAlerts.length > 0) {
      // Re-trigger alarm every 4 seconds while active
  ...
  const triggerAlert = async (type, isCustom = false, source = 'Guest Mobile App', overrideRoom = null) => {
    ...
    let alertData = {
      ...
      summary: `Automated ${type} protocol initiated for ${targetRoom} via ${source}.`,
      translation_es: `Protocolo de ${type} automático iniciado para ${targetRoom} vía ${source}.`,
      tags: [type.toUpperCase(), 'IMMEDIATE RESPONSE', source.includes('IoT') ? 'HARDWARE' : 'HUMAN'],
    ...
    if (isCustom && customText) {
      ...
      alertData = { ...alertData, ...aiAnalysis, originalText: customText, source: 'Guest App + AI' };
    }
  ...
  const Portal = () => {
    ...
    const handleLogin = () => {
      ...
      if (loginType === 'guest') {
        ...
        if (venue) {
          ...
          setError('Invalid Venue Code. Try demo code "VEGAS24"');
        }
    ...
    const seedDemoVenues = async () => {
      if (venues.length === 0) {
        const demoVenues = [{ name: "Grand Horizon Resort", code: "VEGAS24", lat: "36.112634", lng: "-115.176746", rooms: ["301","302","303","304","305","306","307","308"] }];
      ...
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 text-slate-200">
        <div className="w-full max-w-md bg-slate-800 rounded-3xl p-8 shadow-2xl border border-slate-700">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-slate-400 text-sm">Enterprise Emergency Mesh</p>
          </div>

          <div className="flex gap-2 bg-slate-900 p-1.5 rounded-xl mb-6">
            <button onClick={() => {setLoginType('guest'); setError('');}} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-colors ${loginType === 'guest' ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'}`}>Guest</button>
            <button onClick={() => {setLoginType('staff'); setError('');}} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-colors ${loginType === 'staff' ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'}`}>Staff</button>
          ...
          <div className="space-y-4">
            {loginType !== 'admin' && (
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Venue Code</label>
            ...
            <button onClick={handleLogin} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl mt-4 transition-colors flex justify-center items-center gap-2">
               Access Network <ChevronRight size={18} />
            </button>
          </div>
          
          {venues.length === 0 && <button onClick={seedDemoVenues} className="w-full mt-6 text-xs text-slate-500 underline hover:text-slate-300">Seed Demo Data (Run Once)</button>}
        </div>
      </div>
    );
  };
  ...
  const AdminSettings = () => {
    const [formData, setFormData] = useState({ name: '', code: '', lat: '', lng: '', roomsStr: '' });
    const [isLocating, setIsLocating] = useState(false);
    
    // IoT Sim State
    const [iotRoom, setIotRoom] = useState(activeVenue?.rooms?.[0] || '');
    const [iotType, setIotType] = useState('Fire');
    const [iotDevice, setIotDevice] = useState('Smart Smoke Detector');
    ...
    const handleSave = async (e) => {
      ...
      const newVenue = { name: formData.name, code: formData.code.toUpperCase(), lat: formData.lat, lng: formData.lng, rooms: roomsArray };
      try {
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', VENUES_COLLECTION), newVenue);
        setFormData({ name: '', code: '', lat: '', lng: '', roomsStr: '' });
      } catch(err) { console.error(err); }
    };
    ...
    const copyDeepLink = (code) => {
        ...
        alert(`Copied Guest QR Link to clipboard:\n${link}\n\nGuests scanning this QR code bypass login.`);
    };
         ...
         <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               ...
               <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                  <h2 className="text-lg font-bold mb-4 flex items-center gap-2"><Plus size={20}/> Add New Venue</h2>
                  <form onSubmit={handleSave} className="space-y-4">
                     <div>
                       <label className="block text-sm font-bold text-slate-700 mb-1">Venue Name</label>
                       <input required type="text" value={formData.name} onChange={e=>setFormData({...formData, name: e.target.value})} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. Acme Towers" />
                     </div>
                     <div>
                       <label className="block text-sm font-bold text-slate-700 mb-1">Access Code (Unique)</label>
                       <input required type="text" value={formData.code} onChange={e=>setFormData({...formData, code: e.target.value.toUpperCase()})} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none uppercase font-mono" placeholder="e.g. ACME2026" />
                     </div>
                     ...
                     <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                        <div className="flex justify-between items-center mb-2">
                           ...
                           <button onClick={handleAutoDetectGPS} type="button" className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded flex items-center gap-1 hover:bg-blue-200 transition-colors font-bold">
                              {isLocating ? <div className="w-3 h-3 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"/> : <GpsIcon size={12}/>} 
                              Auto-Detect GPS
                           </button>
                        </div>
                        <div className="flex gap-4">
                            <div className="flex-1">
                            <input required type="text" value={formData.lat} onChange={e=>setFormData({...formData, lat: e.target.value})} className="w-full p-2 bg-white border border-slate-300 rounded focus:ring-2 focus:ring-blue-500 outline-none text-sm" placeholder="Lat (e.g. 34.05)" />
                            </div>
                            <div className="flex-1">
                            <input required type="text" value={formData.lng} onChange={e=>setFormData({...formData, lng: e.target.value})} className="w-full p-2 bg-white border border-slate-300 rounded focus:ring-2 focus:ring-blue-500 outline-none text-sm" placeholder="Lng (e.g. -118.2)" />
                            </div>
                        </div>
                     </div>

                     <div>
                       <label className="block text-sm font-bold text-slate-700 mb-1">Rooms/Zones (Comma Separated)</label>
                       <textarea required value={formData.roomsStr} onChange={e=>setFormData({...formData, roomsStr: e.target.value})} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Lobby, 101, 102, Pool" rows={3}></textarea>
                     </div>
                     <button type="submit" className="w-full bg-slate-900 text-white font-bold py-3 rounded-lg hover:bg-slate-800 transition-colors">Create Venue</button>
                  </form>
               </div>
               ...
               <div className="space-y-6">
                  ...
                  <div className="bg-slate-900 p-6 rounded-2xl shadow-xl border border-slate-800 text-slate-200">
                      <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-white"><Cpu size={20} className="text-blue-500"/> IoT Hardware Simulator</h2>
                      
                      {activeVenue ? (
                        <div className="space-y-4">
                            <div className="flex gap-4">
                                <div className="flex-1">
                                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Target Zone</label>
                                ...
                                <div className="flex-1">
                                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Alert Category</label>
                                    <select value={iotType} onChange={e => setIotType(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2 text-white outline-none">
                                        ...
                                        <option value="Medical">Medical / Fall</option>
                                    </select>
                                </div>
                            </div>
                            <button onClick={() => triggerAlert(iotType, false, `IoT Sensor (${iotType})`, iotRoom)} disabled={isSubmitting} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-2.5 rounded-lg mt-2 flex items-center justify-center gap-2">
                                {isSubmitting ? 'Transmitting...' : 'Trigger Sensor'}
                            </button>
                        </div>
                      ) : <div className="text-yellow-500 text-sm">Create a venue first.</div>}
                  </div>

                  {/* Existing Venues */}
                  <div>
                      <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-slate-800"><Building size={20}/> Active Venues</h2>
                      {venues.map(v => (
                        <div key={v.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 mb-4">
                            <div className="flex justify-between items-start">
                              ...
                              <div className="flex gap-2">
                                <button onClick={() => copyDeepLink(v.code)} title="Copy QR/Deep Link" className="text-blue-500 hover:bg-blue-50 p-2 rounded-lg border border-blue-100"><LinkIcon size={18}/></button>
    ...
    if (currentRoomAlert) {
      return (
        <div className="flex flex-col items-center justify-center min-h-screen w-full p-6 text-center bg-slate-900 text-white overflow-y-auto">
          ...
          <p className="text-slate-300 mb-6 max-w-sm text-md">Security teams are en route to {guestRoomId}.</p>
          <div className="bg-slate-800 p-5 rounded-2xl border border-slate-700 w-full max-w-md mb-6">
            <h3 className="font-semibold text-slate-100 mb-3 flex items-center justify-center gap-2"><ShieldAlert size={18} className="text-red-400" /> Protocols</h3>
            <ul className="text-left text-slate-300 space-y-3">
              <li className="flex items-start gap-3 bg-slate-900/50 p-3 rounded-xl border border-slate-700/50 text-sm">
                ...
                <span>Evacuate immediately if you detect smoke. Feel doors before opening.</span>
              </li>
              <li className="flex items-start gap-3 bg-slate-900/50 p-3 rounded-xl border border-slate-700/50 text-sm">
                <CheckCircle2 size={18} className="text-emerald-400 mt-0.5 shrink-0" />
                <span>Unlock doors to allow rapid responder access if safe.</span>
              </li>
            </ul>
          </div>
        ...
        <div className="max-w-md mx-auto min-h-full bg-white shadow-xl flex flex-col">
          ...
          <div className="flex-grow px-6 py-6 space-y-4 bg-slate-50/50">
            ...
            <button onClick={() => triggerAlert('Fire')} disabled={!guestRoomId || isSubmitting} className="group w-full bg-white hover:bg-orange-50 border-2 border-slate-100 hover:border-orange-500 rounded-3xl p-4 flex items-center gap-4 transition-all shadow-sm disabled:opacity-50">
              ...
              <div className="text-left flex-grow">
                <h2 className="text-lg font-bold text-slate-800">Fire / Smoke</h2>
  ...
  // 4. Staff Dashboard (WITH SIREN/VIBRATION)
    ...
    return (
      <div className="flex flex-col md:flex-row h-full w-full bg-slate-100 overflow-hidden font-sans">
        ...
        <div className="hidden md:flex w-20 bg-slate-900 flex-col items-center py-6 shadow-2xl z-20 shrink-0">
          ...
          <div className="mt-auto"><button onClick={handleLogout} className="text-slate-600 hover:text-red-400 transition-colors p-3"><LogOut size={24} /></button></div>
        </div>
        ...
        <div className="w-full md:w-[450px] bg-white border-r border-slate-200 flex flex-col h-[50vh] md:h-full z-10 shadow-xl shrink-0">
          <div className={`p-4 md:p-6 border-b shrink-0 transition-colors ${isRedAlert ? 'bg-red-600 border-red-700 text-white' : 'border-slate-100'}`}>
            <h1 className="text-xl md:text-2xl font-black tracking-tight flex items-center gap-2">
                {isRedAlert && <AlertTriangle size={24} className="animate-pulse" />} Active Incidents
            </h1>
              ...
              <div key={alert.id} className="bg-white p-4 rounded-2xl shadow-sm border-2 border-red-200 hover:border-red-400 transition-all">
                ...
                <div className="bg-slate-50 p-3 rounded-xl mb-3 border border-slate-100">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-slate-500 border-t border-slate-200 pt-2">
                    ...
                    <span className="font-semibold text-slate-600">Source:</span> {alert.source || 'Guest App'}
                  </div>
             ...
             <div className="flex-grow flex items-center justify-center overflow-auto p-4 custom-scrollbar">
                <div className="flex flex-wrap justify-center gap-4 max-w-4xl">
                   {activeVenue.rooms?.map(rId => {
                     ...
                     return (
                        <div key={rId} className={`relative flex flex-col items-center justify-center border-2 rounded-xl transition-all w-24 h-24 md:w-32 md:h-32 ${isAlert ? 'border-red-500 bg-red-50 shadow-[0_0_20px_rgba(239,68,68,0.3)] animate-pulse scale-105 z-10' : 'border-slate-200 bg-slate-50'}`}>
                           <span className={`font-bold font-mono ${isAlert ? 'text-red-700 text-xl' : 'text-slate-500 text-lg'}`}>{rId}</span>
  ...
  // 5. Responder HUD (WITH SIREN/VIBRATION)
  const ResponderView = () => {
    if (!activeResponderAlert) {
      return (
    ...
    return (
      <div className="bg-[#0a0f16] h-full w-full text-slate-300 flex flex-col font-mono selection:bg-red-500/30 overflow-hidden">
        <div className="bg-red-900/20 border-b border-red-500/30 p-3 flex justify-between items-center shadow-lg shrink-0">
          <div className="flex items-center gap-3">
            <div className="bg-red-600 text-white p-2 rounded animate-pulse shrink-0"><AlertTriangle size={20} /></div>
            <div className="min-w-0">
              <div className="text-red-500 font-bold text-sm md:text-xl tracking-widest uppercase truncate">TACTICAL UPLINK</div>
              <div className="text-[10px] md:text-xs text-red-400/70 truncate">TOKEN: {activeResponderAlert.responderLink} • {activeVenue.name}</div>
            </div>
          </div>
          <div className="flex items-center gap-4">
             <div className="text-right shrink-0">
               ...
               <div className="text-[8px] md:text-[10px] text-red-400 uppercase tracking-widest">Est. Arrival</div>
             </div>
        ...
        <div className="p-4 flex-grow flex flex-col lg:flex-row gap-4 w-full max-w-[1600px] mx-auto overflow-hidden">
          <div className="w-full lg:w-1/3 flex flex-col gap-4 overflow-y-auto custom-scrollbar shrink-0 h-[45vh] lg:h-full">
            <div className="bg-slate-900/80 p-5 border border-slate-800 rounded-lg relative overflow-hidden shrink-0">
              ...
              <h3 className="text-red-500 text-[10px] md:text-xs uppercase tracking-[0.2em] font-bold mb-3">Target Location</h3>
              ...
              <h3 className="text-red-500 text-[10px] md:text-xs uppercase tracking-[0.2em] font-bold mb-2 mt-4">Incident Profile</h3>
              <div className="bg-black/50 p-3 rounded border border-slate-800 text-slate-200">
                ...
                <div className="text-[10px] text-slate-500 uppercase flex items-center gap-1 border-t border-slate-800 pt-2">
                   ...
                   Triggered By: {activeResponderAlert.source || 'Guest Mobile App'}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-full lg:w-2/3 bg-slate-900/80 border border-slate-800 rounded-lg flex flex-col relative overflow-hidden shadow-2xl h-[55vh] lg:h-full shrink-0">
            <div className="p-3 border-b border-slate-800 flex justify-between items-center bg-black/80 z-20 shrink-0">
              ...
              <span className="text-green-500 text-[10px] md:text-xs tracking-widest flex items-center gap-1.5"><span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>ACTIVE</span>
            </div>
            
            <div className="flex-grow relative flex bg-black overflow-hidden w-full h-full">
               <iframe 
                 title="Tactical Map"
                 src={`https://maps.google.com/maps?q=${activeVenue.lat},${activeVenue.lng}&t=k&z=18&ie=UTF8&iwloc=&output=embed`}
               ...
               <div className="absolute inset-0 pointer-events-none flex items-center justify-center overflow-hidden">
                  <div className="absolute w-[150%] max-w-[800px] aspect-square bg-gradient-to-r from-transparent via-blue-500/10 to-transparent animate-[spin_4s_linear_infinite] origin-center opacity-60 rounded-full" style={{ background: 'conic-gradient(from 0deg, transparent 0deg, rgba(59, 130, 246, 0.3) 60deg, transparent 60deg)' }}></div>
